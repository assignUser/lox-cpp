...
class Interpreter : public Visitor {
 public:
  Interpreter() : globals{std::make_shared<Environment>()}, m_env(globals) { importStd(); }

  ExprPtr interpret(std::vector<StmtPtr> const& statements);
  ExprPtr interpret(Expr const* expr);
  void resolve(Expr const* const expr, size_t depth) { m_locals.insert_or_assign(expr, depth); }
 ...
 private:
  ...
  class Context {
   ...
   public:
    ...
    Context(Interpreter& interp, tl::optional<std::shared_ptr<Environment>> const& parent)
      ...
      m_interp.m_env = parent.value_or(std::make_shared<Environment>(m_previous));
    }
