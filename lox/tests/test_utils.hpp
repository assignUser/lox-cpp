...
ExprPtr bangUnary(bool value) {
  return Unary::make(Token{Token::Type::BANG, "!"}, Boolean::make(value));
}

ExprPtr minusUnary(double value) {
  if (value < 0) {
    throw std::out_of_range("'Number' can only hold positive values.");
  }

  return Unary::make(Token{Token::Type::MINUS, "-"}, Number::make(value));
}
